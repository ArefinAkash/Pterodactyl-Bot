  ...
  try {
    ...
    if (message.content.startsWith(prefix) && !message.content.startsWith(prefix + " ")) {
      ...
      if (fs.existsSync('./customcmds/' + cmd + ".txt")) {
        ...
        await customCommands(client, message, cmd, code, 0, actualpanellink, focused)
      }
...
async function customCommands(client, message, cmd, code, loopnum, actualpanellink, focused) {
  // Running
              ...
              if (freevar == "goto" || freevar == "else" || freevar.slice(5).trim() == "") {
                ...
                if (isNaN(parseFloat(freevar.slice(5)))) {
                  ...
                  if (freevar.startsWith(output)) {
                    customCommandsEnd(client, message, cmd, code, parseFloat(freevar.slice(5)) - 2, actualpanellink, focused);
                  } else {
                    customCommandsEnd(client, message, cmd, code, loopnum, actualpanellink, focused);
                  }
                }
              }
            } else {
    ...
    if (code[loopnum].trim() == "embed") {
      ...
      if (code[loopnum].trim().slice("embed ".length).replace(/\\n/g, "\n").replace(/%prefix%/g, prefix).replace(/%args%/g, message.content.trim().slice(prefix.length + cmd.length + 1).trim()).length > 2000) {
        ...
        let embed = new Discord.MessageEmbed()
          .setColor(color)
          .setFooter(footername, footerimage)
        let thing = code[loopnum].trim().slice("embed ".length).replace(/\\n/g, "\n").replace(/%prefix%/g, prefix).replace(/%args%/g, message.content.trim().slice(prefix.length + cmd.length + 1).trim())
        if (thing.replace(/\|/g, "") !== thing) {
          title = thing.split("|")[0];
          let msg = thing.slice(title.length + "|".length)
          embed.setTitle(await title)
          embed.setDescription(await msg)
        } else {
          embed.setDescription(await thing)
        }
        await message.channel.send(embed)
        await customCommandsEnd(client, message, cmd, code, loopnum, actualpanellink, focused);
      }
    }
    return;
          ...
          if (code[loopnum + 1].trim().slice(2).trim() == "goto" || code[loopnum + 1].trim().slice(2).trim() == "else") {
            ...
            if (isNaN(parseFloat(code[loopnum + 1].trim().slice(2).trim().slice(5)))) {
              ...
              if (code[loopnum + 1].trim().slice(2).trim().startsWith(output)) {
                customCommandsEnd(client, message, cmd, code, parseFloat(code[loopnum + 1].trim().slice(2).trim().slice(5)) - 2, actualpanellink, focused);
              } else {
                customCommandsEnd(client, message, cmd, code, loopnum + 1, actualpanellink, focused);
              }
            }
          }
        } else {
          ...
          if (isNaN(code[loopnum].trim().slice("ifargs ".length).trim().slice(5).trim())) {
            ...
            if (code[loopnum].trim().slice("ifargs ".length).trim().startsWith("goto ")) {
              if (message.content.trim().slice(prefix.length + cmd.length + 1).trim() == "") {
                ...
                customCommandsEnd(client, message, cmd, code, code[loopnum].trim().slice("ifargs ".length).trim().slice(5).trim() - 2, actualpanellink, focused);
                return;
              }
            } else { // else
              if (message.content.trim().slice(prefix.length + cmd.length + 1).trim() !== "") {
...
async function customCommandsEnd(client, message, cmd, code, loopnum, actualpanellink, focused) {
  if (loopnum + 1 < code.length) {
    //setTimeout(() => {
      customCommands(client, message, cmd, code, loopnum + 1, actualpanellink, focused)
    //}, 1);
  }
}
